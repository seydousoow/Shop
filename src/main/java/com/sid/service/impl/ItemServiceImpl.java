...
public class ItemServiceImpl implements ItemService {
    ...
    public List<String> getAllBrands(String category) {
        ...
        return brands.stream().distinct().sorted().toList();
    }

    @Override
    public Item getItem(String code) {
        Item item = itemRepository.findItemByCodeEquals(code);
        if (item == null) throw new RestException("This article doesn't exist!");
        return item;
    }
    ...
    public void delete(String id) {
        ...
        if (optionalItem.isEmpty())
            throw new RestException("This article does not exist!");
